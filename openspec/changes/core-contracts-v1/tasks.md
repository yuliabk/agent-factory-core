...
- [x] C7.3 Smallest useful real source set is proven without widening public authority: internal evidence plus a real keyless English Wikipedia source through the official MediaWiki REST API. Wikipedia is background-only (`freshness=any`); `recent/current` is not misrepresented. Source timeouts/failures degrade through limitations, deterministic tests pass, a live Wikipedia CI smoke test passes, and Core Registry advances to exact sandbox release `4a8b308aeaf22228c6a03d438509b0717e6daf8b`. Production remains blocked.
- [x] C7.4a Travel Agent is the first sandbox consumer boundary: external Travel commit `9da84b635d1ea3b1d62f4b4e8652acd22e42ead6` contains a provider-neutral `research.lookup` consumer, PII-minimized requests, non-commercial background evidence separation, fail-safe workflow handling and its own green Contract v1 CI. Core locks the external manifest and proves compile-time/ExecutionContext authority contains only `research.lookup`, resolves to the exact Research release, and never grants provider-internal `web.search` or direct tool bindings.
- [ ] C7.4b Execute one real Travel -> Core -> Research sandbox request through a governed capability transport, preserving trace/deadline/budget/audit boundaries. Remote capability transport is not yet defined; do not substitute direct Travel-to-Research URL coupling.
